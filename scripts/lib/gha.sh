...
# Section opener (format B): one styled, collapsible header. GitHub Actions
# renders ANSI color inside `::group::` titles, so the bold-green verb styling
# lives in the title itself — a single emission means the header text can't
# drift, and the title never renders twice in the log.
#
#   gha_section Building "source"
#   → ::group::Building source            (collapsible, bold-green verb)
#
# The verb is a capitalised present participle; the rest is the title. Pair
# with gha_group_end to close the collapsible.
gha_section() {
    local verb="$1"
    shift
    gha_group_begin "$(anodizer::verb_str "${verb}" "$*")"
}
